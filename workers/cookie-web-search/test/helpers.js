 ...
 * object (postgres.js's dynamic `SET ${sql(updates)}` helper) records the
 * touched column names instead of consuming the queue. Calling it with a
 * plain string (postgres.js's `${sql(name)}` dynamic identifier helper)
 * resolves to an escaped-identifier marker without touching the call log or
 * queue, matching the real driver: resolving an identifier isn't a round
 * trip.
 *
 * Mirrors workers/cookie-web-tasks/test/helpers.js exactly.
 *
