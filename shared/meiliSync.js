 ...
 * trips to Postgres and Meilisearch. Selects every row in one query and
 * pushes them in one addDocuments call, the same way the single-message
 * path does for one row. Some ids may not come back (deleted between the
 * mutation and this sync, or never existed) — search_indexed_at is only
 * stamped for the ids that did, same as the single-message path only
 * stamps when it found a row.
 *
 * @param {import('postgres').Sql} sql
 * @param {any} env
...
export async function syncMessagesToMeili(sql, env, messageUuids) {
  if (!messageUuids.length || !meiliAvailable(env)) return;
  ...
  `;
    ...
    const result = await addDocuments(env, MESSAGES_INDEX, rows);
    const syncedIds = rows.map((row) => row.id);
    await sql`UPDATE messages SET search_indexed_at = now() WHERE id = ANY(${syncedIds})`;
    console.log(
      JSON.stringify({
        event: 'meili_synced',
